...
import {
  ...
  test("summarizeToolArgs normalizes spaces and truncates long text", async () => {
    const text = `  hello\n\n${"world ".repeat(80)}  `;
  ...
  test("summarizeToolResult extracts explicit json error messages", async () => {
    const summarized = summarizeToolResult('{"error":"tool failed: timeout"}');
    assert.equal(summarized, "tool failed: timeout");
  });

  test("summarizeToolResult handles terminal and object results", async () => {
  ...
  test("extractToolContent and summarizeResultText keep core text semantics", async () => {
