...
import {
  ...
  test("create session defaults prefer stored valid mode", () => {
  ...
  test("create session defaults fall back to the first available mode", () => {
    assert.equal(resolveCreateSessionModeId({
  ...
  test("create session defaults expose mode-specific placeholders", () => {
  ...
  test("create session defaults read and write localStorage mode memory", () => {
