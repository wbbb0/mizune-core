...
import { readFile } from "node:fs/promises";
  ...
  test("session state panel keeps code blocks scrollable and no longer edits titles inline", async () => {
    ...
    assert.match(source, /TranscriptDisclosure/);
    assert.match(source, /collapsed-label="派生观察"/);
    assert.match(source, /collapsed-label="最近工具事件"/);
    assert.match(source, /collapsed-label="最近发送记录"/);
