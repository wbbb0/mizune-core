...
import { readFile } from "node:fs/promises";
  ...
  test("session state panel keeps code blocks scrollable and no longer edits titles inline", async () => {
    ...
    assert.match(source, /collapsed-title="当前 Bot 身份"/);
    assert.match(source, /botProfileRows/);
    assert.match(source, /当前聊天未设置 Bot 身份覆盖，将使用全局 persona 和当前模式资料。/);
    assert.match(source, /修改入口为聊天内的会话身份工具/);
