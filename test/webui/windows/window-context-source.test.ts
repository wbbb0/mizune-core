...
test("workbench windows expose generic resource context without session-specific framework code", async () => {
  const [typesSource, managerSource, sessionsContextSource, chatPanelSource] = await Promise.all([
    readFile(new URL("../../../vendor/workbench-kit/packages/vue-workbench/src/windows/types.ts", import.meta.url), "utf8"),
    readFile(new URL("../../../vendor/workbench-kit/packages/vue-workbench/src/windows/windowManager.ts", import.meta.url), "utf8"),
