...
test("desktop layout keeps safe-area padding on the workbench shell, not the activity bar", async () => {
  ...
  assert.match(composerSource, /useWorkbenchViewport/);
  assert.match(composerSource, /paddingBottom:\s*isMobile\.value && keyboardInsetPx\.value === 0/);
  ...
  assert.match(composerSource, /:accept="COMPOSER_FILE_ACCEPT"/);
  assert.match(composerSource, /:accept="COMPOSER_IMAGE_ACCEPT"/);
  assert.match(composerSource, /function toggleAttachmentMenu\(\)/);
  assert.match(composerSource, />图片</);
  assert.match(composerSource, />文件</);
  assert.match(composerSource, /function uploadComposerFiles\(files: File\[\]\)/);
