...
test("materialized evidence is bounded and truncates large canonical content", () => {
  const hugeCanonical = "x".repeat(13_000);
  const tracker = service.materializeEvidenceBeforeCompression({
    sessionId: "s1",
    tracker: trackerWithPrimary(),
    transcriptItemsToCompress: Array.from({ length: 70 }, (_, index) => ({
      ...
      content: "{}",
      canonicalContent: hugeCanonical,
      observation: {
        contentHash: `hash-${index}`,
        inputTokensEstimate: 20,
        summary: `写入文件 ${index}`,
        retention: "summary",
        replayContent: `{\"ok\":true,\"index\":${index}}`,
        replaySafe: true,
        refetchable: false,
        pinned: false
      }
  ...
  assert.equal(tracker.evidence.length, 64);
  assert.equal(tracker.evidence[0]?.toolCallId, "call-6");
  assert.equal(tracker.evidence.at(-1)?.canonicalContent?.length, 12_000);
  assert.equal(tracker.evidence.at(-1)?.canonicalTruncated, true);
});

test("materialized evidence can belong to parked task without primary", () => {
  const tracker = service.materializeEvidenceBeforeCompression({
    sessionId: "s1",
    tracker: {
      version: 1,
      ...
      parked: [{
        ...
        importantToolRefs: [{
          ...
          toolName: "terminal_start",
          resource: { kind: "shell_session", id: "term-parked" },
          createdAtMs: 1
        }],
        updatedAtMs: 2
      }],
    ...
    transcriptItemsToCompress: [{
      ...
      toolCallId: "call-parked",
      toolName: "terminal_read",
      content: JSON.stringify({ session: { status: "closed", resource_id: "term-parked" }, exitCode: 0 }),
      observation: {
        contentHash: "hash-parked",
        inputTokensEstimate: 10,
        summary: "后台终端已完成",
        retention: "summary",
        replayContent: "{\"exitCode\":0}",
        resource: { kind: "shell_session", id: "term-parked" },
        replaySafe: true,
        refetchable: false,
        pinned: true
  ...
  assert.equal(tracker.evidence[0]?.taskId, "parked-task");
  assert.equal(tracker.evidence[0]?.toolCallId, "call-parked");
});

test("materialized evidence prefers parked task match over current primary", () => {
  const tracker = service.materializeEvidenceBeforeCompression({
    sessionId: "s1",
    tracker: {
      ...trackerWithPrimary({
