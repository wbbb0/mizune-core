...
- 主目录工作副本默认视为稳定运行副本；需要在主目录验证新代码时，应在开发分支完成一个可验证切片后，再显式合并或切换到主目录验证。
- 默认采用以下联调流程：
  - 在开发用 worktree 中完成修改
  - 先在开发 worktree 中跑必要的 typecheck / test
  - 先做一个临时 commit（允许后续整理 commit 历史）
  - 再将该 commit 或分支合并到主 worktree，让主目录 dev 运行新代码进行联调
  - 联调通过后，再继续整理、补充或合并后续 commit
