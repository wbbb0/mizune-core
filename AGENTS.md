...
- 默认采用以下联调流程：
  ...
  - 先做一个 `wip` 临时 commit，用于合并到主 worktree 做本地联调或验收
  - 再将该 `wip` commit 或分支合并到主 worktree，让主目录 dev 运行新代码进行联调
  - 主目录验证通过后，再把本次开发历史整理为最终保留的正式提交；默认不要让多个 `wip` 提交长期留在 `main` 历史里
