...
- `Data`：查看和编辑常见运行数据 JSON，例如全局 persona、RP / Scenario 全局资料、users、whitelist、scheduled jobs
...
- 聊天页与后台记录页都支持通过右键或移动端长按打开消息操作菜单，可删除单条记录或整组 turn 产物。
- 删除不会物理移除 transcript item，而是将其标记为失效：
  - 聊天页会立即隐藏失效项
  - 后台记录页仍保留失效项，并以半透明、禁用交互的形式展示
- transcript 流同步采用“新增 + 补丁”模型，已有消息的思考内容补写和失效状态也会通过 SSE 实时更新到前端。
