...
export const minecraftActorRecoveryStateSchema = s.object({
...
export const runtimeResourceRecordSchema = s.object({
  ...
  kind: s.enum(["browser_page", "shell_session", "minecraft_actor"] as const).title("资源类型"),
  ...
  minecraftActor: minecraftActorRecoveryStateSchema.title("Minecraft Actor").optional()
}).title("资源")
...
export const runtimeResourceFileSchema = s.object({
  resources: s.array(runtimeResourceRecordSchema)
    .title("资源列表")
    .describe("当前保存的浏览器页面、Shell 会话和 Minecraft Actor 恢复记录。")
    .default([])
}).title("运行时资源")
