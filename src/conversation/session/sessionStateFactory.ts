...
export function restoreSessionState(item: PersistedSessionState): SessionState {
  ...
  return {
    ...
    pendingSteerMessages: [],
    pendingReplyGateWaitPasses: 0,
    queuedGroupReplyTargets: (item.queuedGroupReplyTargets ?? []).map((target) => ({
      userId: target.userId,
      senderName: target.senderName,
      ...(target.groupId ? { groupId: target.groupId } : {}),
      transcriptGroupId: target.transcriptGroupId,
      firstAtMs: target.firstAtMs,
...
export function toPersistedSessionState(session: SessionState): PersistedSessionState {
  return {
    ...
    queuedGroupReplyTargets: session.queuedGroupReplyTargets.map((target) => ({
      userId: target.userId,
      senderName: target.senderName,
      ...(target.groupId ? { groupId: target.groupId } : {}),
      transcriptGroupId: target.transcriptGroupId,
