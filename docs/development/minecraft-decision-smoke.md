...
2026-08-13 使用生产 DeepSeek provider 的实测结果：

- `deepseek-v4-flash` 关闭思考，三次 behavior 分别约为 6.23、6.80、6.84 秒，reasoning token 均为 0。
- `deepseek-v4-pro` 关闭思考，完整 program 读取、生成、校验和激活约为 10.21 秒，reasoning token 为 0。

因此普通事件决策继续使用十秒级快路径；完整程序发布属于低频路径，应使用更宽截止时间，不能阻塞反射或已运行的确定性状态机。
