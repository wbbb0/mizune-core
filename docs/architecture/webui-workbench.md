...
本文档整理当前 WebUI 工作台的长期结构约定，覆盖 section workbench、全局 chrome / 菜单、toast、统一窗口系统以及 pane sizing。
...
- 项目侧 `SectionHost`
  - 根据路由选择 section，并把 section、导航项和导航回调交给 workbench
- `WorkbenchShell`
  - 统一挂载桌面端 / 移动端工作台外壳
  - 同时挂载全局 `MenuHost`、`ToastViewport` 与 `WindowHost`
- 项目侧 section registry
  - 维护本项目 section 注册表，不属于 workbench framework 核心
- `workbenchRuntime`
...
- `webui/src/sections/SectionHost.vue`
...
- `webui/src/sections/useSectionRegistry.ts`
...
- 导航项由项目侧传入，workbench core 不直接 import 业务路由或业务 section 列表
...
- pane 的实际宽度由 `workbenchRuntime` 的 pane registry 统一维护
- 当前已有 `list` pane；后续扩展 `right` / `bottom` pane 应继续接入同一个 registry
- 用户拖拽后的 pane 宽度以全局 pane id 持久化，而不是按 section 分开保存
- section 切换时，runtime 会用当前 section 的 `minWidthPx` / `maxWidthPx` 对全局宽度重新 clamp
- 双击分隔条重置为当前 section 的默认宽度，并写回全局宽度
- 后续若扩展到更多 VS Code 风格 pane，继续沿用“全局实际尺寸 + section 约束”的模型
...
- 页面导航菜单由项目侧传入的导航项生成，workbench core 不依赖 Vue Router
...
Toast 属于 workbench overlay 能力，由 `WorkbenchShell` 挂载，而不是由 `App.vue` 全局挂载。

当前约定：

- 业务代码通过 `useWorkbenchToasts` 发送 toast
- `ToastViewport` 由 `WorkbenchShell` 统一渲染
- toast 样式依赖 workbench 主题 token，不直接依赖业务页面结构

相关实现入口：

- `webui/src/components/workbench/toasts/useWorkbenchToasts.ts`
- `webui/src/components/workbench/toasts/ToastViewport.vue`
...
- `webui/src/components/workbench/windows/WorkbenchDialog.vue`
...
抽成独立模板项目时，应把这些 token 作为模板主题契约保留，业务项目只覆盖 token 值，不改 workbench 组件结构。
