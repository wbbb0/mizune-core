...
会话详情的“设置”页统一管理模型可用工具边界、模型路由与回复节奏。工具边界限制 Turn Planner 的候选工具集以及模型本轮最终可见的业务工具；它不会强制模型使用已允许的工具，也不能绕过身份、模型能力和系统配置限制，工具集查询与申请两个管理元工具始终保留。会话仅保存相对于模式默认值的覆盖，因此以后新增工具集会自然采用其声明的默认值；模式初始化等完成当前操作所必需的工具不受普通会话边界限制。

“允许模型自行升级”默认开启。只有当前轮仍使用 `mainSmall`，并且 `mainSmall` 与 `mainLarge` 的全部 fallback 候选均属于同一个 provider、都是支持工具的聊天模型，且两个首选项实际模型不同时，轻量模型才会看到 `request_model_upgrade`。该控制工具不由 Turn Planner 选择，也不属于任何业务工具集；调用后只把同一工具循环的后续 provider 请求切到 `mainLarge`，不会重新运行 Planner、改变身份权限或扩大本轮业务工具集合。关闭开关或修改设置从下一轮生成起生效。

回复节奏可以覆盖用户消息聚合等待，并可单独开关 OneBot 模型回复的模拟输入延迟。OneBot 会话默认使用自适应输入聚合和 `conversation.outbound` 延迟参数；新建 Web 会话默认立即处理输入，回复形成正式分段后也会立即显示。复制会话到 Web 时会完整继承源会话的全部设置。Turn Planner 主动等待更多输入的 `gate_wait` 不受会话级输入聚合设置影响。
...
- `state/state.sqlite`，其中包含 `persona`、`rp_profile`、`global_profile_readiness`、`setup_state`、`users`、`user_memories`、`pending_requests`、`scheduled_jobs`、`scheduled_job_targets`、`global_rules`、`toolset_rules`、`toolset_rule_toolsets`、`user_identities`、`group_membership_entries` 与 `whitelist_entries`
...
迁移会归并旧版 memory / rules 数据，并生成 `memory-migration-report.json` 审计报告。`persona`、RP 全局资料、全局资料就绪状态、初始化状态、用户资料、规则、身份映射、群成员缓存、待处理请求、定时任务和白名单已由 `state/state.sqlite` 承载；会话快照与 `scenario_host` 每会话状态已由 `sessions/sessions.sqlite` 承载，Scenario 资料随对应会话状态保存。对应旧 JSON 文件不再作为运行时数据源。
