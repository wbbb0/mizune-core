...
会话详情的“设置”页统一管理模型可用工具边界与回复节奏。工具边界限制 Turn Planner 的候选工具集以及模型本轮最终可见的业务工具；它不会强制模型使用已允许的工具，也不能绕过身份、模型能力和系统配置限制，工具集查询与申请两个管理元工具始终保留。会话仅保存相对于模式默认值的覆盖，因此以后新增工具集会自然采用其声明的默认值；模式初始化等完成当前操作所必需的工具不受普通会话边界限制。
